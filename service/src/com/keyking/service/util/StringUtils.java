...
public class StringUtils {
	public static final String[] toLowerCaseWordArray(String str) {
		...
		for (int i = 0; i < strs.length; ++i)
			strs[i] = token.nextToken().toLowerCase();
		return strs;
	}
	...
	public static String[] splitOnWhitespace(String paramString) {
		...
		for (int k = 0; k < j; ++k) {
			...
			if (!Character.isWhitespace(c))
				continue;
			if (k - i > 1)
				lis.add(paramString.substring(i + 1, k));
			i = k;
		}
		return lis.toArray(new String[lis.size()]);
	}

	public static final String replaceAll(String paramString1,
			String paramString2, String paramString3) {
		if ((paramString1 != null) && (paramString2 != null)
				&& (paramString3 != null) && (!paramString1.equals(""))
				&& (!paramString2.equals(""))) {
			...
			while ((i < j)
					&& ((k = paramString1.indexOf(paramString2, i)) >= i)) {
				localStringBuilder.append(paramString1.substring(i, k)).append(
						paramString3);
				i = k + l;
			}
			if (i == 0)
	...
	public static final String subString(String paramString, int paramInt) {
		...
		if (paramString.length() <= paramInt)
			return paramString;
		return new StringBuilder(paramString.substring(0, paramInt)).append(
				"...").toString();
	}

	public static final String join(String paramString1, String paramString2,
	...
	public static boolean isNull(String paramString1) {
